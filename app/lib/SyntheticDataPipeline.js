  ...
  async process(text) {
    ...
    try {
      ...
      this.onProgress?.({
        ...
        message: `Classifying ${Math.min(extractedClauses.length, 200)} clauses`,
        progress: 60
      });
  ...
  async _extractClauses(chunks) {
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        ...
        this.onProgress?.({
          stage: 'extraction',
          message: `Processing batch ${Math.floor(i/BATCH_SIZE) + 1} of ${Math.ceil(chunks.length/BATCH_SIZE)}, with ${batchChunks.length} chunks`,
  ...
  async _classifyClauses(clauses) {
    ...
    try {
      ...
      for (let i = 0; i < clauses.length; i += BATCH_SIZE) {
        ...
        this.onProgress?.({
          stage: 'classification',
          message: `Processing classification batch ${Math.floor(i/BATCH_SIZE) + 1} of ${Math.ceil(clauses.length/BATCH_SIZE)}, with ${batchClauses.length} clauses`,
  ...
  async _generateVariants(classifiedClauses) {
    ...
    try {
      ...
      for (let i = 0; i < classifiedClauses.length; i += BATCH_SIZE) {
        ...
        this.onProgress?.({
          stage: 'generation',
          message: `Processing variant batch ${Math.floor(i/BATCH_SIZE) + 1} of ${Math.ceil(classifiedClauses.length/BATCH_SIZE)}, with ${batchClauses.length} clauses`,
