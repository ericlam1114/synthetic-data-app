 ...
 */
function maintainJobLock(jobContext, currentProgress, message, intervalMs = 10000) {
  ...
  const interval = setInterval(() => {
    ...
    jobContext.progress(
      currentProgress, 
      `${message} (maintaining connection - ${new Date().toISOString()})`
    );
  }, intervalMs);
...
export async function processDocumentInBackground(
  ...
  try {
    ...
    const stopHeartbeat = maintainJobLock(jobContext, 25, "Running AI processing", 15000);
